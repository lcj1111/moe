...
本手册不是阶段结果的简单拼接，而是项目的“操作系统说明书”。建议分三遍阅读：

1. 第一遍只看第 1–5 章，建立整体架构、当前状态和事实边界；
2. 第二遍看第 6–10 章，理解代码、数据、Gate 和每条主线的实现；
3. 第三遍按第 14 章做接管演练，不要一开始就启动 8 卡正式实验。
...
以下是 2026-08-13 接管手册生成时的状态。运行中数字会变化，查询方法见第 11 章。
...
| NVFP4 full-set | 971 条续跑完成，`failed=0`，仍有 18 条截断待审计 | 尚不可发布最终准确率 |
| FP8 full-set | 等 NVFP4 合并 Gate 后运行 | 尚未开始 |
| BF16 full-set | 旧任务被外部 SIGTERM 中断 | 未闭合 |
...
3. NVFP4 full-set 截断续跑完成、合并和 Gate 生成之前，不发布其最终准确率。
