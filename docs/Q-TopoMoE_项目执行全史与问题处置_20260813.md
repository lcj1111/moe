...
| Phase 3 NVFP4 full-set | 基础轮完成；971 条截断续跑中 | 进行中 |
| Phase 3 FP8 full-set | 等 NVFP4 合并 Gate 后启动 | 排队 |
| Phase 3 BF16 full-set | 旧任务被外部 SIGTERM 中断 | 未闭合 |
...
1. 等 NVFP4 971 条续跑完成，审计 `failed/truncated/finish_reason`，按 ID 合并并生成
   24,374 条唯一结果、摘要和 SHA-256；若仍截断，仅对可解释的残留做有限二次补跑。
2. 使用完全相同的冻结输入运行 FP8 TP2 双分片，执行同一截断补跑和合并 Gate。
3. 若最终结论需要“相对本机同协议 BF16”而不只是模型卡参考，重新完成 BF16 full-set。
4. 为 Phase 8 selector 增加真实 KV-cache 命中、队列深度、近期到达率和短窗口 p99；
   冻结特征、模型与阈值后新增独立测试 workload，重新计算 median/p95 regret。
5. 只有 selector 全部 Gate 通过后，才执行动态 trigger、cooldown、rollback 在线闭环。
6. 最后更新 README、数据清单、失败矩阵、复现顺序和一键 smoke，验证所有链接与哈希，
   再创建 release tag。
