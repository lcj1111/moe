...
### 3.4 2026-08-07：正式 P2P 系统基线

1. 8×8 peer read/write 非对角项全部为 `OK`，NCCL 使用
   `P2P/direct pointer`。
2. 完成 NCCL、BF16/FP8 服务、阶段 3 pilot、阶段 6 TP/DP/EP 和阶段 7
   迁移成本的同环境正式测量。
3. 通信成本库表明 PIX TP2 适合作为首选通信映射；端到端服务仍按 workload
   矩阵复核。
4. 对 W4A16 做 group128→group64 的无损重分块，逐值验证 `max diff=0`，解决
   TP8 分片对 block 对齐的格式限制。
5. 完成四卡和八卡 TP/DP/EP smoke 矩阵与迁移成本微基准。
